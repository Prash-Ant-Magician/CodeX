...
interface TypescriptRunnerProps {
    code: string;
    setCode: (code: string) => void;
}

export default function TypescriptRunner({ code, setCode }: TypescriptRunnerProps) {
  ...
  const handleCompileAndRun = async () => {
    ...
    setResult(null);
    try {
      const output = await runTypescriptCode({ code });
      setResult(output);
    } catch (error) {
      console.error(error);
  ...
  const handleSuggestCode = async () => {
    ...
    setSuggestion('');
    try {
      const result = await suggestCode({ code, language: 'typescript' });
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:h-[calc(100vh-10rem)]">
        <Card className="flex flex-col h-[60vh] md:h-full">
            <CardHeader>
                <CardTitle>TypeScript Editor</CardTitle>
                <CardDescription>Write and compile TypeScript code. The AI will simulate compilation and execution.</CardDescription>
            </CardHeader>
