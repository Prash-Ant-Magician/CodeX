...
export default function RRunner({ code, setCode }: RRunnerProps) {
  ...
  const handleSuggestCode = async () => {
    ...
    setSuggestion('');
    try {
      const result = await suggestCode({ code, language: 'r' });
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:h-[calc(100vh-10rem)]">
        <Card className="flex flex-col h-[60vh] md:h-full">
            <CardHeader>
                <CardTitle>R Language Editor</CardTitle>
                <CardDescription>Write and run R code. The AI will simulate execution.</CardDescription>
            </CardHeader>
